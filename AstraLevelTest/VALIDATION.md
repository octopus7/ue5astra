...
`/Game/Astra/Maps/L_AstraStarfall`을 별도 100.8×100.8m Landscape 레벨로 저장했다. 16도 기울어진 우주선, 큰 웅덩이 3곳, 신선한 파단목과 오래된 통나무, 일반 버섯 15×15m, 발광 버섯 5×5m, 작은 옹달샘과 분홍 잎 나무 7그루를 포함한다.

- Blender 모델 키트 3개를 병렬 제작하고 13개 모델의 FBX 재로드에서 크기·삼각형·재질 슬롯·UV·작성 노멀을 검증했다. 수면 4개를 더해 새 메시 17개이며 기존 메시 16개를 재사용한다.
- 새 맵은 액터 704개, 개별 메시 배치 370개, native Foliage 4,118개다. 총 4,488개 배치 기록과 일치한다.
- 전체 UE 에디터의 저장 맵 재로드 검사 통과. Landscape 내부 15,625개 샘플, 작성 노멀, 재질 슬롯과 섹션, 배치·회전·크기, 몸통 충돌, 7개 데모 경로를 확인했다. Foliage 위치 오차 0cm, 최대 회전 오차 약 0.0000153도, 배율 오차 약 0.0000000596이다.
- 우주선 세 발은 최대 0.00233cm 높이 차이로 지면에 접한다. 기울기는 모델에 구워 넣어 액터에서 중복 회전하지 않는다.
- 기존 Content 254개 파일의 SHA256이 그대로다. 기존 스카이와 반사 머터리얼, 광원 Source Angle 50°, Fog/Post Process 설정을 재사용했다. 새 지면의 물밑 색만 조정했다.
- C++ Editor 빌드 통과. 실제 `-AstraDemoTest`는 81.456초의 월드 시간, 실패 0개로 종료했다. 7개 경로 모두 끝까지 걸었으며 지면 지지가 끊긴 시간은 0초다. P 진입·루프·즉시 취소·복귀 및 상태 복원, 복귀 후 WASD 네 방향 이동 332.3~338.1cm를 확인했다.
- 최종 검토 렌더 7장과 실제 P 데모 렌더 7장을 저장했다. 새 레퍼런스 4장·선체 페인트 1장과 함께 PNG CRC·압축 데이터·화면 내용 및 해시를 확인했다. 최종 실행 로그에 머터리얼 컴파일 오류가 없다.

[전달 산출물 검사](ArtSource/Previews/StarfallDelivery_Validation.json), [저장 맵 검사](ArtSource/Previews/UE_StarfallReloadValidation.json), [실제 이동 검사](ArtSource/Previews/UE_SFDemoValidation.json). [구성과 재생성 절차](Docs/StarfallLevel.md).

기존 기본 맵과 Windows 배포 ZIP은 유지했다. 이번 검증은 에디터 및 UnrealEditor의 독립 게임 실행이며 새 레벨을 넣은 배포 패키지를 다시 만들지는 않았다.
...
기존 숲의 생성 이미지 27장(전체 참고 2장, 개별 모델 참고 19장, 텍스처 6장)을 원본 PNG로 보관했다. [레퍼런스 목록](ArtSource/Reference/README.md)과 각 생성 프롬프트를 함께 저장했다. 별내림 숲의 새 참고 이미지 4장과 선체 페인트 1장도 `Reference/Starfall`, `Textures/Starfall`에 원본과 프롬프트를 추가 보관했다.
