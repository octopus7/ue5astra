"""Build only L_AstraStarPond from the exported Blender layout in full UE 5.7.

The woodland map and all non-StarPond Content files are read-only inputs.
...
MATERIAL_DIR = '/Game/Astra/Materials/StarPond'
MESH_DIR = '/Game/Astra/Meshes/StarPond'
TEXTURE_DIR = '/Game/Astra/Textures/StarPond'
FOLIAGE_DIR = '/Game/Astra/Foliage/StarPond'
...
def protected_content():
    ...
    for path in (ROOT/'Content').rglob('*'):
        ...
        if 'StarPond' in relative.parts or path.stem.startswith('L_AstraStarPond'):
            continue
...
def new_material(name):
    if not name.startswith('M_SP_'):
        raise RuntimeError('New materials must use M_SP_: '+name)
...
def build_materials(data):
    ...
    for name, props in data['materials'].items():
        if name == 'M_SP_Landscape':
            continue
        if props.get('reuse_existing') or not name.startswith('M_SP_'):
    ...
    result['M_SP_Landscape'] = build_ground_material()
    import starpond_water
    from types import SimpleNamespace
    result['M_SP_StarWater'] = starpond_water.build(SimpleNamespace(**globals()))
    return result


def build_ground_material():
    material = new_material('M_SP_Landscape')
    ...
    code = (ART/'Layout/starpond_ground.hlsl').read_text(encoding='utf-8')
...
def import_meshes(data, materials):
    ...
    for name, item in data['assets'].items():
        ...
        result[name] = mesh
        unreal.log('STARPOND imported '+name)
    return result
...
def clear_new_level(data):
    ...
    file = ROOT/'Content/Astra/Maps/L_AstraStarPond.umap'
    if file.exists():
        backup = ART/'Backups/BeforeStarPondRebuild'; backup.mkdir(parents=True,exist_ok=True)
        if not (backup/file.name).exists():
            shutil.copy2(file, backup/file.name)
        if not levels.load_level(MAP):
            raise RuntimeError('Could not load existing StarPond map')
    elif not levels.new_level_from_template(MAP,TEMPLATE):
        raise RuntimeError('Could not create independent StarPond map from template')
    ...
    if current != baseline:
        raise RuntimeError('Template environment settings differ in StarPond')
    return world, baseline


def spawn(actor_class, location, label, rotation=None, folder='StarPond'):
...
def place_level(data, materials, meshes, world):
    landscape = unreal.AstraSceneLibrary.create_terrain_from_heightmap(
        materials['M_SP_Landscape'],str(ROOT/data['terrain_file']))
    if not isinstance(landscape,unreal.Landscape):
        raise RuntimeError('StarPond native Landscape creation failed')
    landscape.set_actor_label('Landscape_100m_StarPond'); landscape.set_folder_path('Terrain')
    ...
    for item in data['objects']:
        ...
        if item.get('collision') == 'trunk':
            ...
            radius = meta.get('radius_m',.33 if item['asset'].startswith('SM_SP_Pink') else .28)
            height = meta.get('height_m',2.8 if item['asset'].startswith('SM_SP_Pink') else 2.4)
            ...
            blocker = spawn(unreal.StaticMeshActor,xyz(location),item['name']+'_TrunkCollision',
                            item['ue_rotation_deg'],'Collision/StarPondTrunks')
    ...
    spawn(unreal.PlayerStart,data['spawn_cm'],'PlayerStart_StarPond',{'yaw':155},'Gameplay')
    config = spawn(unreal.AstraLevelConfig,[0,0,0],'StarPondLevelConfig',folder='Gameplay')
    shots = []
    for item in data['demo_shots']:
        ...
        shots.append(shot)
    config.set_editor_property('demo_shots',shots); config.set_editor_property('validation_prefix','SP')
    for item in data['review_cameras']:
        ...
        actor=spawn(unreal.CameraActor,position,name,{'pitch':pitch,'yaw':yaw},'ReviewCameras/StarPond')
    ...
    for item in data.get('lights',[]):
        actor=spawn(unreal.PointLight,item['ue_location_cm'],item['name'],folder='Lighting/StarPondGlow')
    ...
    # Invisible shoreline guard follows the irregular bank and blocks only pawns.
    import starpond_layout as design
    cube=EAL.load_asset('/Engine/BasicShapes/Cube')
    for i in range(96):
        a=i*math.tau/96;b=(i+1)*math.tau/96
        p=[design.RX*math.cos(a)*design.rim(a)*.96,design.RY*math.sin(a)*design.rim(a)*.96]
        q=[design.RX*math.cos(b)*design.rim(b)*.96,design.RY*math.sin(b)*design.rim(b)*.96]
        actor=spawn(unreal.StaticMeshActor,[(p[0]+q[0])*50,(p[1]+q[1])*50,160],f'SP_WaterBoundary_{i:03}',
            {'yaw':math.degrees(math.atan2(q[1]-p[1],q[0]-p[0]))},'Collision/PondEdge')
        c=actor.static_mesh_component;c.set_mobility(unreal.ComponentMobility.MOVABLE);c.set_static_mesh(cube)
        actor.set_actor_scale3d(unreal.Vector(math.dist(p,q)+.10,.24,4.))
        c.set_collision_profile_name('Custom')
        ...
        c.set_visibility(False);c.set_cast_shadow(False);actor.set_actor_hidden_in_game(True)
        c.set_mobility(unreal.ComponentMobility.STATIC)
    label=next(o['name'] for o in data['objects'] if o['asset']=='SM_SP_StarPondSurface')
    water=next(a for a in unreal.get_editor_subsystem(unreal.EditorActorSubsystem).get_all_level_actors() if a.get_actor_label()==label)
    mystery=spawn(unreal.AstraStarPond,[0,0,0],'StarPondDiscovery',folder='Gameplay')
    mystery.set_editor_property('water_actor',water)
    mystery.set_editor_property('observatory',unreal.Vector(-1600,0,148))
    import import_starpond_elephant
    importlib.reload(import_starpond_elephant)
    guardian, guardian_report=import_starpond_elephant.import_and_place(data['elephant']['ue_location_cm'],data['elephant']['yaw'])
    return foliage


def main():
    data=json.loads((ART/'Layout/starpond_layout.json').read_text(encoding='utf-8'))
    (ART/'Previews').mkdir(parents=True,exist_ok=True)
    before=protected_content()
    baseline_file=ROOT/'Saved/AstraBuild/StarPondProtection.json';baseline_file.parent.mkdir(parents=True,exist_ok=True)
    ...
    try:
        ...
        if '-AstraStarPondPlacementOnly' in unreal.SystemLibrary.get_command_line():
            materials={'M_SP_Landscape':build_ground_material()}
        ...
        if not unreal.get_editor_subsystem(unreal.LevelEditorSubsystem).save_current_level():
            raise RuntimeError('Could not save StarPond map')
        ...
        report.update({'passed':all(unchanged.values()),'old_content_unchanged':all(unchanged.values()),
             ...
             'foliage':foliage,'actor_count':len(actors),'layout_sha256':digest(ART/'Layout/starpond_layout.json'),
             'terrain_sha256':digest(ROOT/data['terrain_file']),'saved_only_new_map_and_starpond_assets':True})
        ...
        (ART/'Previews/UE_StarPondImportValidation.json').write_text(json.dumps(report,indent=2),encoding='utf-8')
    unreal.log('STARPOND IMPORT COMPLETE '+json.dumps({k:v for k,v in report.items() if k not in ('protected_asset_sha256','environment_state')}))
