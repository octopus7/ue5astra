"""Build only L_AstraRootBelltower from the exported Blender layout in full UE 5.7.

The woodland map and all non-RootBelltower Content files are read-only inputs.
Existing meshes, floor texture and foliage types are reused as-is. Lighting
changes belong only to the new map; all other Content files are hashed.
...
MATERIAL_DIR = '/Game/Astra/Materials/RootBelltower'
MESH_DIR = '/Game/Astra/Meshes/RootBelltower'
TEXTURE_DIR = '/Game/Astra/Textures/RootBelltower'
FOLIAGE_DIR = '/Game/Astra/Foliage/RootBelltower'
...
def protected_content():
    ...
    for path in (ROOT/'Content').rglob('*'):
        ...
        if 'RootBelltower' in relative.parts or path.stem.startswith('L_AstraRootBelltower'):
            continue
...
def new_material(name):
    if not name.startswith('M_RB_'):
        raise RuntimeError('New materials must use M_RB_: '+name)
...
def build_materials(data):
    ...
    for name, props in data['materials'].items():
        if name == 'M_RB_Landscape':
            continue
        if props.get('reuse_existing') or not name.startswith('M_RB_'):
        ...
        if props.get('emissive_color') and props.get('emissive_strength',0):
            ...
            if name=='M_RB_SapGlow':
                parameter=expression(material,unreal.MaterialExpressionScalarParameter)
                parameter.set_editor_property('parameter_name','Resonance')
                parameter.set_editor_property('default_value',0.)
                emissive=custom(material,'return Tint*(1.+2.4*Resonance);',{'Tint':emissive,'Resonance':parameter})
    ...
    result['M_RB_Landscape'] = build_ground_material()
    return result


def build_ground_material():
    material = new_material('M_RB_Landscape')
    ...
    code = (ART/'Layout/rootbelltower_ground.hlsl').read_text(encoding='utf-8')
...
def import_meshes(data, materials):
    ...
    for name, item in data['assets'].items():
        ...
        settings.remove_degenerates = False  # Source kits already remove zero-area faces.
        ...
        result[name] = mesh
        unreal.log('ROOTBELLTOWER imported '+name)
    return result
...
def clear_new_level(data):
    ...
    file = ROOT/'Content/Astra/Maps/L_AstraRootBelltower.umap'
    if file.exists():
        backup = ROOT/'Saved/RootBelltowerBackups'; backup.mkdir(parents=True,exist_ok=True)
        if not (backup/file.name).exists():
            shutil.copy2(file, backup/file.name)
        if not levels.load_level(MAP):
            raise RuntimeError('Could not load existing RootBelltower map')
    elif not levels.new_level_from_template(MAP,TEMPLATE):
        raise RuntimeError('Could not create independent RootBelltower map from template')
...
def spawn(actor_class, location, label, rotation=None, folder='RootBelltower'):
...
def place_level(data, materials, meshes, world):
    landscape = unreal.AstraSceneLibrary.create_terrain_from_heightmap(
        materials['M_RB_Landscape'],str(ROOT/data['terrain_file']))
    if not isinstance(landscape,unreal.Landscape):
        raise RuntimeError('RootBelltower native Landscape creation failed')
    landscape.set_actor_label('Landscape_100m_RootBelltower'); landscape.set_folder_path('Terrain')
    ...
    for item in data['objects']:
        ...
        component.set_mobility(unreal.ComponentMobility.MOVABLE if item.get('movable') else unreal.ComponentMobility.STATIC)
        if item.get('collision') == 'trunk':
            meta = data['assets'][item['asset']].get('trunk_collision',{})
            radius = meta.get('radius_m',.33 if item['asset'].startswith('SM_RB_Pink') else .28)
            height = meta.get('height_m',2.8 if item['asset'].startswith('SM_RB_Pink') else 2.4)
            ...
            blocker = spawn(unreal.StaticMeshActor,xyz(location),item['name']+'_TrunkCollision',
                            item['ue_rotation_deg'],'Collision/RootBelltowerTrunks')
    ...
    spawn(unreal.PlayerStart,data['spawn_cm'],'PlayerStart_RootBelltower',{'yaw':155},'Gameplay')
    config = spawn(unreal.AstraLevelConfig,[0,0,0],'RootBelltowerLevelConfig',folder='Gameplay')
    shots = []
    for item in data['demo_shots']:
        ...
        shots.append(shot)
    config.set_editor_property('demo_shots',shots); config.set_editor_property('validation_prefix','RB')
    for item in data['review_cameras']:
        pitch,yaw = item.get('pitch',-58),item.get('yaw',0)
        p,y = math.radians(pitch),math.radians(yaw)
        look=item['look_cm']; arm=item.get('arm',4200 if item['width']>5000 else min(2700,item['width']*.75))
        ...
        actor=spawn(unreal.CameraActor,position,name,{'pitch':pitch,'yaw':yaw},'ReviewCameras/RootBelltower')
        ...
        if 'near_clip_cm' in item:
            camera.set_editor_property('auto_calculate_ortho_planes',False)
            camera.set_editor_property('ortho_near_clip_plane',item['near_clip_cm'])
            camera.set_editor_property('ortho_far_clip_plane',10000.)
    for item in data.get('lights',[]):
        actor=spawn(unreal.PointLight,item['ue_location_cm'],item['name'],folder='Lighting/RootBelltowerGlow')
    ...
    for actor in actors:
        if isinstance(actor,unreal.DirectionalLight):
            actor.light_component.set_intensity(3.5)
            actor.light_component.set_editor_property('light_source_angle',50.)
        elif isinstance(actor,unreal.SkyLight):actor.light_component.set_intensity(.75)
        elif isinstance(actor,unreal.ExponentialHeightFog):
            actor.component.set_editor_property('fog_density',.002)
            actor.component.set_editor_property('fog_max_opacity',.12)
            actor.component.set_editor_property('start_distance',2500.)
    audio_dir='/Game/Astra/Audio/RootBelltower'
    sound_task=unreal.AssetImportTask();sound_task.filename=str(ART/'Audio/RootBelltower/A_RB_BronzeChime.wav')
    sound_task.destination_path=audio_dir;sound_task.destination_name='A_RB_BronzeChime'
    sound_task.automated=True;sound_task.save=True;sound_task.replace_existing=True
    AT.import_asset_tasks([sound_task]);sound=EAL.load_asset(audio_dir+'/A_RB_BronzeChime')
    assert isinstance(sound,unreal.SoundWave),'Bell chime import failed'
    mystery=spawn(unreal.AstraRootBelltower,[0,0,0],'RootBelltowerResonance',folder='Gameplay')
    mystery.set_editor_property('bell_actor',next(a for a in actors if a.get_actor_label()==data['bell_label']))
    mystery.set_editor_property('root_actors',[a for a in actors if isinstance(a,unreal.StaticMeshActor) and any(a.static_mesh_component.get_material(i) and a.static_mesh_component.get_material(i).get_name()=='M_RB_SapGlow' for i in range(a.static_mesh_component.get_num_materials()))])
    mystery.set_editor_property('resonance_center',unreal.Vector(*data['resonance_center_cm']))
    mystery.set_editor_property('chime_sound',sound)
    return foliage


def main():
    data=json.loads((ART/'Layout/rootbelltower_layout.json').read_text(encoding='utf-8'))
    (ART/'Previews').mkdir(parents=True,exist_ok=True)
    before=protected_content()
    baseline_file=ROOT/'Saved/AstraBuild/RootBelltowerProtection.json';baseline_file.parent.mkdir(parents=True,exist_ok=True)
    ...
    try:
        ...
        if '-AstraRootBelltowerPlacementOnly' in unreal.SystemLibrary.get_command_line():
            materials={'M_RB_Landscape':build_ground_material()}
        ...
        if not unreal.get_editor_subsystem(unreal.LevelEditorSubsystem).save_current_level():
            raise RuntimeError('Could not save RootBelltower map')
        ...
        report.update({'passed':all(unchanged.values()),'old_content_unchanged':all(unchanged.values()),
             ...
             'environment_state':environment_state(actors),'template_environment_state':environment,'new_meshes':sum(not a.get('reuse_existing') for a in data['assets'].values()),
             ...
             'foliage':foliage,'actor_count':len(actors),'layout_sha256':digest(ART/'Layout/rootbelltower_layout.json'),
             'terrain_sha256':digest(ROOT/data['terrain_file']),'saved_only_new_map_and_rootbelltower_assets':True})
        ...
        (ART/'Previews/UE_RootBelltowerImportValidation.json').write_text(json.dumps(report,indent=2),encoding='utf-8')
    unreal.log('ROOTBELLTOWER IMPORT COMPLETE '+json.dumps({k:v for k,v in report.items() if k not in ('protected_asset_sha256','environment_state')}))
